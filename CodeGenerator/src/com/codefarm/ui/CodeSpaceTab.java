...
public class CodeSpaceTab extends JPanel implements Observer {
	...
	public CodeSpaceTab(FrontEndContext frontEnd, int codeSpaceId) {
		...
		_csId = codeSpaceId;
		if (_frontEnd.getCodeSpaceOwnerType(_csId) == Code.USER_TYPE) _isUserOwned = true;
		else _isUserOwned = false;
		if (_isUserOwned) _csOwner = _frontEnd.getUser(_frontEnd.getCodeSpaceOwnerId(_csId)).toString() + " (User)";
	...
	private void generateCodes() {
		...
		if (n != null) {
			...
			_frontEnd.addCodes(Integer.valueOf(n), _csId, _frontEnd.getCodeSpaceOwnerId(_csId));
			refreshCodesList();
		}
	}
	...
	private void showCodeDataDialog() {
		...
		if (_frontEnd.getCodesByCodeSpaceId(_csId).size() == 0) return;
		SetCodeDataDialog scdd = new SetCodeDataDialog(_csId, Code.CODE_SPACE_TYPE, _frontEnd);
	...
	private void initialise() {
		...
		if (_frontEnd.getCodeSpaceOwnerType(_csId) == Code.FRONTEND_TYPE) {
		...
		JLabel csNameLabel = new JLabel(_frontEnd.getCodeSpaceName(_csId));
		JLabel codeLenLabel = new JLabel(String.valueOf(_frontEnd.getCodeSpaceCodeLen(_csId)));
		JLabel excludesLabel = new JLabel(String.valueOf(_frontEnd.getCodeSpaceExcludes(_csId)));
		_codesCountLabel = new JLabel(String.valueOf(_frontEnd.getCodesByCodeSpaceId(_csId).size()));
	...
	private void refreshCodesList() {
		...
		for (Code code : _frontEnd.getCodesByCodeSpaceId(_csId)) {
			String userInfo;
